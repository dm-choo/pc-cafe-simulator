...
좌석은 자체 제작 GLB를 설치한 수만큼 공유 인스턴싱합니다. 목재·콘크리트·벽에는 Poly Haven CC0 1K PBR 재질을 공유합니다. `npm run dev/build`에서 GLB를 생성하고 원본 해시를 확인한 재질을 캐시/다운로드합니다. 최초 빌드에 약 3.96MB의 재질 다운로드가 필요하며 배포 후에는 모두 같은 사이트에서 로드됩니다. [자산 출처](public/assets/material-credits.txt), [제작 절차](docs/art/pipeline.md). 인물은 임시 모델이며 최종 아트·실 GPU 최적화는 진행 중입니다.
