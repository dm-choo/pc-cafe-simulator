...
**현재 개발: 0석 창업과 실사 기반 재질.** 빈 임대 공간에서 카운터·좌석을 직접 구매하고 설치한 뒤 첫 영업을 시작합니다. 저장은 아직 없으며 새로고침하면 초기화됩니다. 저장소는 [dm-choo/pc-cafe-simulator](https://github.com/dm-choo/pc-cafe-simulator)입니다. 실제 검증·배포 상태는 [현재 상태](docs/production/status.md)에 기록합니다.
...
- 범위: 같은 게임을 0→12→24석·7일 완성판으로 만든 뒤 60석으로 확장합니다.
...
| 이번 작업의 범위·완료 조건 | [0석 창업 작업서](docs/tasks/004-zero-seat-opening.md) |
...
첫 시작은 **0석·카운터 없음·설비 재고 없음**, 시험 창업 자금 400만원입니다. N에서 카운터 세트(40만원)와 기본 좌석 세트(120만원)를 사고, B에서 카운터와 원하는 좌석 구역을 설치하세요. 둘러보기로 돌아가 카운터를 바라보고 E → 영업 시작을 누릅니다. 구매는 재고만 늘리고 설치해야 가구와 충돌이 생깁니다.

현재는 12개 고정 좌석 구역 중 선택하는 설치 방식입니다. 자유 이동·회전·철거는 아직 없습니다. 영업 중이거나 손님이 남아 있으면 설치할 수 없고, 사장이 설치 위치에 서 있어도 차단됩니다. 탭을 숨기거나 UI를 열면 시간이 멈춥니다. 현재는 PC 키보드·마우스용입니다.
...
창업 자금·설비 가격·이용료는 모두 **시험 밸런스이며 2026-01-01 실가격 검증값이 아닙니다**. 기본 좌석은 실존 SKU가 아닌 임시 패키지입니다. 첫 이용은 1시간 1,500원이며 약 40 sim초 후 자동 결제됩니다. 카운터의 신규 입장 중지는 기존 손님의 이용·결제를 취소하지 않습니다.

좌석은 자체 제작 GLB를 설치한 수만큼 공유 인스턴싱합니다. 목재·콘크리트·벽에는 Poly Haven CC0 1K PBR 재질을 공유합니다. `npm run dev/build`에서 GLB를 생성하고 원본 해시를 확인한 재질을 캐시/다운로드합니다. 최초 빌드에 약 5.96MB의 재질 다운로드가 필요하며 배포 후에는 모두 같은 사이트에서 로드됩니다. [자산 출처](public/assets/material-credits.txt), [제작 절차](docs/art/pipeline.md). 인물은 임시 모델이며 최종 아트·실 GPU 최적화는 진행 중입니다.
