...
const admit = () =>
  command(command(equipped(), { type: "open" }), { type: "arrive" });
describe("첫 영업", () => {
  ...
  it("입장과 함께 좌석을 예약하고 만석에서는 중복 배정하지 않는다", () => {
    let state = command(equipped(12), { type: "open" });
  ...
  it("조기 종료를 거부하고, 이용 종료 결제는 재입력해도 한 번만 반영한다", () => {
    ...
    expect(state.revenue).toBe(TRIAL_PRICE_WON);
  ...
  it("1배속과 4배속은 같은 tick에서 고객·좌석·매출이 같다", () => {
    function run(speed: number, duration: number) {
      let state = command(equipped(), { type: "open" });
