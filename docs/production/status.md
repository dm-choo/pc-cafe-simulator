...
- 로컬 규칙 18개, lint·타입 검사·빌드 통과. [첫 CI](https://github.com/dm-choo/pc-cafe-simulator/actions/runs/34096526769)는 브라우저 3개 통과, BASIC-12/FIRST-SALE 두 긴 시나리오가 전체 120초 제한에 걸렸습니다. 검사를 생략하거나 시간을 늘리지 않고 정지 장면의 불필요한 렌더링과 낮음 해상도를 개선해 재검사합니다.
- 빈 공간·구매·설치·첫 착석 화면을 직접 검토했습니다. 바닥 얼룩 대비가 너무 강해 더 차분한 회색 CC0 재질로 교체했습니다. 재검사 및 새 공개 배포는 진행 중입니다.
...
이번 0석 변경의 CI 브라우저·화면 검토 후 Pages에 배포합니다. 이어서 저장·이어하기와 고객 요구에 따른 첫 투자 선택을 연결합니다. G003 인물·실 GPU 검증도 유지합니다. 최초 Pages 설정을 다시 요구하지 않습니다.
