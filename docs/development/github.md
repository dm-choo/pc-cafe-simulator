...
G001의 실행·검사 명령은 README와 AGENTS에 기록합니다. Node 버전과 package-lock을 사용합니다. 필요한 쓰기 인증은 개발 환경에서 설정하며 토큰을 문서·채팅에 붙여 넣지 않습니다.
...
G001의 Game checks 워크플로가 npm ci·lint·규칙 테스트·타입 포함 빌드·BASIC-12를 실행하고 화면과 빌드를 artifact로 보관합니다. PR에서는 검사와 빌드만 하고 검증되지 않은 변경이 공개 플레이 URL을 바꾸지 않도록 합니다. 실제 배포 권한은 배포 job에만 둡니다.
